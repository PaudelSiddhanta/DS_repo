...
// starter-code-ends
//===========================================================================================
// exception class QueueEmpty when the queue is empty
...
//exception class QueueFull when the queue is full
...
{
	// to-do
	array = new T[QUEUE_SIZE];	//create a dynamic array of size QUEUE_SIZE
	capacity = QUEUE_SIZE;	//assign capacity to QUEUE_SIZE
	size = 0;		//initialize the size, front and rear variables to 0 because they are just constructed
	front = 0;
	rear = 0;
}
//===========================================
...
{
	// to-do
	delete[] array;			//delete the array from heap under completion of program
}
//===========================================
template <typename T>
...
{
	// to-do
	if(isFull()){				//if the queue is full, element can't be added
		throw QueueFull();
	}	
	array[rear] = element;			//add new element to the rear of the array
	rear = (rear + 1) % capacity;		//update the index of the rear and size of queue
	size++;
}
//===========================================
template <typename T>
...
{
	// to-do
	if(isEmpty()){				//if the queue is empty, elements can't be removed
		throw QueueEmpty();
	}
	int temp = front;			//else, update the new initial point for the queue and return the element that was removed
...
{
	// to-do
	return size == 0;			//if the queue is empty, its size will be zero
}
//===========================================
template <typename T>
bool Queue<T>::isFull()
{
	// to-do
	return size == capacity;		//the size will be equal to capacity if the queue is full
}
//=============================================